...
BEGIN
  ...
  IF t.status NOT IN ('pending','confirmed') THEN
    RAISE EXCEPTION 'Bukti hanya bisa diupload saat menunggu pembayaran (sekarang: %)', t.status;
  END IF;

  UPDATE public.marketplace_transactions
  SET bukti_transfer_url = _bukti_url,
      status = 'confirmed',
      updated_at = now()
  WHERE id = _trx_id;
  ...
  SELECT ur.user_id,
         '💰 Bukti transfer baru',
         'Pembeli mengirim bukti pembayaran untuk pesanan #' || substr(t.id::text,1,8) || '. Mohon verifikasi.',
         'approval', '/admin/escrow',
         'marketplace_transactions', t.id
  FROM public.user_roles ur
  WHERE ur.role IN ('super_admin','ketua','bendahara')
...
DECLARE
  t public.marketplace_transactions%ROWTYPE;
  v_fee_persen numeric := 5;
  v_fee_nominal numeric := 0;
...
BEGIN
  ...
  END IF;
  IF NULLIF(t.bukti_transfer_url, '') IS NULL THEN
    RAISE EXCEPTION 'Bukti pembayaran belum diupload';
  END IF;

  SELECT COALESCE(NULLIF(value #>> '{}', '')::numeric, 5)
  INTO v_fee_persen
  FROM public.settings
  WHERE key = 'marketplace_fee_persen';

  v_fee_persen := COALESCE(v_fee_persen, 5);
  v_fee_nominal := ROUND(COALESCE(t.total, 0) * v_fee_persen / 100);
  v_seller_amount := GREATEST(COALESCE(t.total, 0) - v_fee_nominal, 0);

  IF t.seller_id IS NULL THEN
    RAISE EXCEPTION 'Penjual transaksi tidak valid';
  END IF;

  v_seller_wallet := public.get_or_create_wallet(t.seller_id);

  PERFORM 1 FROM public.wallets WHERE id = v_seller_wallet FOR UPDATE;

  UPDATE public.wallets
  SET saldo_escrow = COALESCE(saldo_escrow, 0) + v_seller_amount,
      updated_at = now()
  ...
  VALUES
    (v_seller_wallet, t.seller_id, 'in', v_seller_amount, 'escrow_in',
     'marketplace_transactions', t.id,
  ...
  SET status = 'paid',
      ...
      fee_persen = v_fee_persen,
      fee_nominal = v_fee_nominal,
...
VALUES ('marketplace_fee_persen', '5'::jsonb, 'Persentase fee marketplace untuk koperasi', true)
