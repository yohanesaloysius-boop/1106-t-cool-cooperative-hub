...
export const Route = createFileRoute("/_authenticated/gereja/pengadaan/$id")({
  head: () => ({ meta: [{ title: "Detail PR — Pengadaan Gereja" }] }),
  component: PRDetail,
});
...
function PRDetail() {
  const { id } = useParams({ from: "/_authenticated/gereja/pengadaan/$id" });
  const { user } = useAuth();

  const { data: pr } = useQuery({
    queryKey: ["church-pr", id],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("church_purchase_requests" as any)
        .select("*, church_divisions(nama), church_pr_items(*), church_purchase_orders(*, church_vendors(nama)), church_pr_payments(*), church_pr_receipts(*), church_pr_audit(*)")
        .eq("id", id).single();
  ...
  const audit = (pr.church_pr_audit ?? []).sort((a: any, b: any) => +new Date(a.created_at) - +new Date(b.created_at));
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-3">
        <Button asChild variant="ghost" size="sm">
          <Link to={isOwner ? "/gereja/pengadaan" : "/admin/gereja/pengadaan"}><ArrowLeft className="h-4 w-4" /> Kembali</Link>
        </Button>
      ...
      <Card>
        ...
        <CardContent className="space-y-3 text-sm">
          <div className="grid sm:grid-cols-3 gap-3">
            <Info label="Divisi" value={pr.church_divisions?.nama ?? "—"} />
      ...
      {(pr.church_purchase_orders ?? []).length > 0 && (
        <Card>
          ...
          <CardContent className="space-y-2 text-sm">
            {pr.church_purchase_orders.map((po: any) => (
              <div key={po.id} className="rounded-md border p-3">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="font-medium">{po.church_vendors?.nama}</p>
      ...
      {(pr.church_pr_payments ?? []).length > 0 && (
        <Card>
          ...
          <CardContent className="space-y-2 text-sm">
            {pr.church_pr_payments.map((p: any) => (
