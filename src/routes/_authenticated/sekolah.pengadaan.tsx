...
function PengadaanSekolahPage() {
  const { user } = useAuth();
  const qc = useQueryClient();
  const [open, setOpen] = useState(false);

  const { data: prs, isLoading } = useQuery({
    queryKey: ["my-school-pr", user?.id],
    queryFn: async () => {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-3">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <NewPRDialog onDone={() => { setOpen(false); qc.invalidateQueries({ queryKey: ["my-school-pr"] }); }} />
        </Dialog>
      </div>
        ...
        <CardContent>
          {isLoading ? (
            ...
            <div className="space-y-2">
              {prs.map((pr) => (
                <Link key={pr.id} to="/sekolah/pengadaan/$id" params={{ id: pr.id }} className="block">
                  <div className="flex items-center justify-between gap-3 rounded-lg border p-3 hover:bg-muted/40 transition-colors">
                    <div className="min-w-0">
                      ...
                      <div className="text-xs text-muted-foreground flex items-center gap-2 mt-0.5">
                        ...
                        <span>{pr.school_divisions?.nama ?? "—"}</span>
                        <span>·</span>
...
function NewPRDialog({ onDone }: { onDone: () => void }) {
  ...
  const { data: divisions } = useQuery({
    queryKey: ["school-divisions-active"],
    queryFn: async () => {
      const { data } = await supabase.from("school_divisions" as any).select("id,nama").eq("is_active", true).order("nama");
      return (data ?? []) as any[];
    },
  });
  ...
  const submit = async (asDraft: boolean) => {
    if (!judul.trim() || !divisionId) return toast.error("Lengkapi judul & unit");
    if (!items.length || items.some((i) => !i.nama.trim())) return toast.error("Lengkapi nama barang");
    setSaving(true);
    try {
      const { data: pr, error } = await supabase.from("school_purchase_requests" as any).insert({
      ...
      const { error: e2 } = await supabase.from("school_pr_items" as any).insert(itemRows);
      if (e2) throw e2;
          ...
          <div>
            ...
            <Select value={divisionId} onValueChange={setDivisionId}>
              ...
              <SelectContent>
                ...
                {!divisions?.length && <SelectItem value="_" disabled>Belum ada unit (minta pengurus)</SelectItem>}
              </SelectContent>
            </Select>
          </div>
          <div>
